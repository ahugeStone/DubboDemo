 ...
 */
@Slf4j
//@Activate(group = {Constants.CONSUMER, Constants.PROVIDER}, order = -9000)
public class TestFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        log.info("开始filter");
        log.info("Attachments:" + context.getAttachments().toString());
        log.info("MethodName:" + context.getMethodName());
        log.info("Argument:" + context.getArguments()[0].toString());
        Result result = invoker.invoke(invocation);
        log.info("结束filter，耗时：" + (System.currentTimeMillis() - startTime) + "ms");
        log.info("Attachments:" + context.getAttachments().toString());
        return result;
    }
}
